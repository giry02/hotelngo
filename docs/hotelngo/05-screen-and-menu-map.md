...
| 영역 | 메뉴/기능 |
|---|---|
| 기본 메뉴 | `여행 발견 / 여행기 / 여행 만들기 / 호텔 / 즐길거리 / AI 여행 / 내 여행` |
| 여행 발견 | 목적지 미정 영감, 도시·테마·공식 스토리·랜드마크 |
| 여행기 | 회원이 공개한 여행 일정과 장면을 보고 좋아요·댓글·스크랩·공유하거나 내 여행으로 복사 |
| 여행 일정 | 목적지·날짜·인원을 정하고 숙소·이동·식사·랜드마크·골프·스파·투어를 일자별로 조합 |
| 호텔 | 목적지·숙박일·인원에 맞는 객실·요금·재고·정책 비교와 여행 일정 연결 |
| 즐길거리 | 선택한 목적지의 랜드마크, 골프, 맛집·카페, 마사지·스파, 투어·체험, 렌터카·픽업 |
| AI 여행 | 자연어 조건으로 수동 일정과 동일한 다카테고리 `TripDraft`를 만들고 같은 편집기에서 수정 |
| 내 여행 | 회원 소유 초안·예약 준비·확정·지난 여행·공유 여행과 바우처 관리 |
...
| 화면 | 기능 |
|---|---|
| 일정 기본정보 | 목적지, 출귀국일, 인원, 제목, 여행 성향을 먼저 정하고 숙박 수보다 하루 많은 날짜 열 생성 |
| 일정 목록/상세 | 날짜별 타임라인, 지도, 이동시간, 예약/장소/메모, 순서 변경, 공유/복제 |
| 일정 편집 | 한 날짜에 숙소·이동·식사·랜드마크·골프·스파·투어를 복수 추가하고 일자·시간 이동·교체·삭제 |
| 일정 검증 | 숙소·식사·공항 이동 누락, 시간 충돌, 운영시간, 장거리 이동, 가격·재고 신뢰 상태 경고 |
...
| 여행 발견 | `discover.html` | 목적지 미정 영감과 목적지 선택 후 도시별 스토리·랜드마크·일정 진입을 분리 |
| 여행 일정 | `trip-planner.html` | 목적지·날짜·인원, 5개 DAY, 7개 업종 카탈로그, 다중 항목 추가·이동·삭제·비용·충돌·저장 |
| 여행기 | `community.html`, `trip-guide-detail.html` | 별도 1차 메뉴에서 다른 여행자의 여행기를 보고 소셜 반응을 남기거나 내 여행으로 복사 |
...
| 즐길거리 | `experiences.html` | 목적지를 먼저 선택한 뒤 해당 도시의 랜드마크·식사·골프·스파·투어·이동 후보 탐색 |
...
| AI 여행 | `ai-travel.html` | 자연어에서 호텔·픽업·식사·골프·스파·투어를 포함한 규칙형 Mock 초안을 만들고 공통 편집기로 이동 |
| 내 여행 | `trips.html` | 로그인 전용 여행 목록·상태와 `trip-planner.html` 이어서 편집 진입 |
...
| 사용자 경로 | 파일 | 현재 상태 |
|---|---|---|
| 회원 여행기 피드 | `community.html` | 회원 공개 일정, 작성자·태그·좋아요·댓글·스크랩·공유·복사 |
| 회원 여행기 상세·복사 | `trip-guide-detail.html` | 날짜별 코스, 작성자 팔로우, 댓글, 로그인 후 독립 초안 복사 |
...
## 14. 2026-07-24 계획 기준선 화면 구현 현황

- 제품 화면은 `sample.html`과 브랜드 비교 샘플을 제외하고 총 193개다.
...
- `trip-planner.html`을 추가해 한 여행에 숙소·이동·식사·랜드마크·골프·스파·투어를 날짜·시간별로 복수 배치하는 단일 편집기로 통합했다.
- 상단의 `여행기`는 회원 공개 콘텐츠를 발견·참여·복사하는 커뮤니티이고, `여행 만들기`는 내 일정을 편집하는 작업 공간으로 분리한다. 중복된 `현지 장소` 상단 메뉴는 제거하고 실제 업체·상품 데이터는 `즐길거리`에 흡수했다.
