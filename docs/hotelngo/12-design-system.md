...
- 실제 정보를 전달하는 텍스트의 최소 크기: `12px`. 배지·메타 정보·사진 수·확인 시각도 이보다 작게 만들지 않는다.
- 메타·보조 정보: `12px`, 카드 본문·필드 라벨: `13~14px`, 일반 버튼: `14px`, 검색·핵심 CTA: `14~16px`
- 본문과 설명문은 `13~16px`를 기본으로 사용하고, `12px`는 짧은 메타·보조 정보에만 사용한다.
- 일반 UI 굵기는 `400 / 500 / 600 / 700` 네 단계로 제한하며, `800` 이상은 로고처럼 고정된 브랜드 표현에만 사용한다.
