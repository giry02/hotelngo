(() => {
  ...
  target.outerHTML = `<aside class="bo-sidebar"><a class="bo-brand" href="index.html"><img src="assets/brand/official/hotelngo-logo-go-capsule-ibm-plex-on-dark.svg?v=1" alt="HotelnGo"><span>${config.label}</span></a>${nav}</aside><header class="bo-topbar"><div class="bo-topbar-left"><button class="bo-button bo-menu" type="button" data-bo-menu>메뉴</button><strong>${config.context}</strong><span>${app === 'channel' ? 'JSON Fixture' : 'Mock 운영'}</span></div><div class="bo-topbar-actions"><a href="index.html">B2C 바로가기</a><a href="${app === 'hotel' ? 'hotel-detail.html' : app === 'channel' ? 'admin-system.html' : 'channel-dashboard.html'}">${app === 'hotel' ? '공개 미리보기' : app === 'channel' ? '플랫폼 관리자' : 'PMS 채널 운영'}</a>${accountActions}</div></header>`;
