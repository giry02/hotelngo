(() => {
  ...
  const destinationTabs = () => {
    const saved = store.groups();
    const known = [...store.destinations, ...catalog.destinations.map((item) => ({ id: item.id, name: item.name }))];
    saved.forEach((group) => { if (!known.some((item) => item.id === group.id)) known.push({ id: group.id, name: group.name }); });
    return known.filter((item, index) => known.findIndex((candidate) => candidate.id === item.id) === index);
  };
  const destinationMeta = (id = activeDestination) => destinationTabs().find((item) => item.id === id) || { id, name: id === 'unassigned' ? '지역 미지정' : id };
  const catalogDestination = () => catalog.destinations.find((item) => item.id === activeDestination) || { ...destinationMeta(), items: [], template: [], recommendedNights: 4 };
  ...
  const openOptions = (item) => {
    ...
    dialog.innerHTML = `<form method="dialog"><header><div><small>${labels[item.category] || '여행 장소'} 설정</small><h2>${escapeHtml(item.title)}</h2></div><button value="cancel" aria-label="닫기">×</button></header><div class="trip-card-dialog-body"><img src="${escapeHtml(item.image)}" alt=""><p>${escapeHtml(item.description || '일정에 담을 조건을 선택해 두면 추천 동선을 만들 때 우선 반영합니다.')}</p><label class="trip-card-location-field"><span>어느 여행에 사용할까요?</span><select name="destinationId">${destinationTabs().filter((destination) => destination.id !== 'unassigned').map((destination) => `<option value="${escapeHtml(destination.id)}" ${destination.id === item.destinationId ? 'selected' : ''}>${escapeHtml(destination.name)}</option>`).join('')}</select><small>지역을 바꾸면 해당 여행지 보관함으로 이동합니다.</small></label><div class="trip-card-option-grid">${fields.map(([label, options], index) => `<label><span>${label}</span><select name="option${index}">${options.map((value) => `<option ${item.options?.[`option${index}`] === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label>`).join('')}</div><div class="trip-card-boundary"><strong>지금은 여행 선호를 정하는 단계입니다.</strong><span>객실 재고·최종 가격·취소 조건은 실제 예약할 때 다시 확인합니다.</span></div></div><footer><button class="ui-button" value="cancel">취소</button><button class="ui-button primary" value="save">저장</button></footer></form>`;
    document.body.append(dialog);
    dialog.addEventListener('close', () => {
      if (dialog.returnValue === 'save') {
        const values = Object.fromEntries(new FormData(dialog.querySelector('form')).entries());
        const destinationId = values.destinationId;
        delete values.destinationId;
        const destination = destinationMeta(destinationId);
        store.update(item.id, { destinationId, destination: destination.name, options: values });
        activeDestination = destinationId;
        selectionInitialized = false;
        render();
      }
  ...
  const render = () => {
    ...
    if (!destinationTabs().some((item) => item.id === activeDestination)) activeDestination = all[0]?.destinationId || 'danang';
    const items = records();
    ...
    root.innerHTML = `<header class="trip-card-hero"><div><span class="page-eyebrow">MY TRAVEL CARD</span><h1>가고 싶은 곳을 모으고,<br>원할 때 일정으로 만드세요</h1><p>여행 카드는 예약 목록이 아니라 나만의 여행 재료 보관함입니다. 장소를 담는 것만으로 일정이 자동 생성되지는 않습니다.</p><div class="trip-card-build-steps"><span><b>1</b>장소 담기</span><span><b>2</b>일정 만들기 선택</span><span><b>3</b>지도·시간 수정</span></div></div><a class="ui-button" href="discover.html">장소 더 둘러보기</a></header>
      <nav class="trip-card-destinations" aria-label="여행지별 보관함">${destinationTabs().map((destination) => `<button type="button" class="${destination.id === activeDestination ? 'is-active' : ''}" data-destination="${destination.id}"><strong>${escapeHtml(destination.name)}</strong><span>${all.filter((item) => item.destinationId === destination.id).length}개 담음</span></button>`).join('')}</nav>
      <section class="trip-card-layout"><div class="trip-card-content">
        <div class="trip-card-location-note"><strong>${escapeHtml(destinationMeta().name)} 여행만 따로 만들어요</strong><span>이 보관함의 장소끼리 이동 가능한 범위를 계산하며, 다른 지역은 별도 일정으로 관리합니다.</span></div>
        <div class="trip-card-section-head"><div><span class="page-eyebrow">SAVED FIRST</span><h2>${escapeHtml(destinationMeta().name)}에서 내가 담은 곳</h2><p>내가 담은 랜드마크를 우선 배치하고 빈 시간은 다른 여행자 데이터와 HotelnGo 추천으로 채웁니다.</p></div><span class="trip-card-count">${items.length}곳</span></div>
        ${items.length ? `<div class="trip-card-items">${items.map((item) => `<article class="trip-card-item ${selected.has(item.id) ? 'is-selected' : ''}"><label class="trip-card-select"><input type="checkbox" data-select="${escapeHtml(item.id)}" ${selected.has(item.id) ? 'checked' : ''}><span>일정 초안에 포함</span></label><img src="${escapeHtml(item.image)}" alt=""><div class="trip-card-item-copy"><small>${icons[item.category] || '•'} ${labels[item.category] || item.category} · ${escapeHtml(item.area)}</small><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description || '담아둔 장소를 추천 동선에 우선 반영합니다.')}</p><div><b>${escapeHtml(displayPrice(item))}</b><span>${item.options && Object.keys(item.options).length ? '선호 옵션 저장됨' : '기본 조건'}</span></div></div><div class="trip-card-item-actions"><button class="ui-button" type="button" data-options="${escapeHtml(item.id)}">지역·옵션 변경</button><button class="text-button" type="button" data-remove="${escapeHtml(item.id)}">삭제</button></div></article>`).join('')}</div>` : `<div class="trip-card-empty"><span>＋</span><h3>아직 ${escapeHtml(destinationMeta().name)}에서 담은 곳이 없어요</h3><p>처음부터 하나씩 고르지 않아도 됩니다. 아래 추천 여행을 통째로 담은 뒤 빼거나 바꿔보세요.</p></div>`}
      ...
      </div><aside class="trip-card-summary"><span class="page-eyebrow">${escapeHtml(destinationMeta().name.toUpperCase())} ROUTE</span><h2>${escapeHtml(destinationMeta().name)} 일정 초안 만들기</h2><div class="trip-card-summary-counts"><div><span>랜드마크</span><strong>${countBy('LANDMARK')}</strong></div><div><span>숙소</span><strong>${countBy('STAY')}</strong></div><div><span>서비스</span><strong>${items.length - countBy('LANDMARK') - countBy('STAY')}</strong></div></div><p>이 지역에서 ${selected.size}곳을 선택했습니다. 다른 여행지에 담은 장소는 이 일정에 섞이지 않습니다.</p><div class="trip-card-estimate"><span>기본 표시 금액 합계</span><strong>${estimated.toLocaleString('ko-KR')}원</strong><small>객실·인원·서비스 옵션에 따라 달라집니다.</small></div><a class="ui-button primary${selected.size && activeDestination !== 'unassigned' ? '' : ' is-disabled'}" href="${selected.size && activeDestination !== 'unassigned' ? `trip-planner.html?destination=${activeDestination}&mode=recommended&fromCard=1&cardIds=${encodeURIComponent(selectedSourceIds)}` : '#'}" ${selected.size && activeDestination !== 'unassigned' ? '' : 'aria-disabled="true"'}>${activeDestination === 'unassigned' ? '먼저 장소의 여행지를 정해주세요' : selected.size ? `${escapeHtml(destinationMeta().name)} 추천 일정 만들기` : '장소를 선택하면 일정 만들기 가능'}</a><a class="ui-button" href="trip-create.html?destination=${encodeURIComponent(destinationMeta().name)}">날짜·인원부터 정하기</a><div class="trip-card-booking-boundary"><strong>실제 예약은 따로 진행합니다</strong><p>일정이 완성된 뒤 객실·티타임·프로그램을 선택하면 예약 카트로 이동합니다.</p><a href="booking-cart.html">예약 카트 보기</a></div></aside></section>`;
  };
